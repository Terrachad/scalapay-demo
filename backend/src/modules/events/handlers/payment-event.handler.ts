...
export class PaymentEventHandler {
  ...
  async handlePaymentCompleted(event: PaymentCompletedEvent): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Payment analytics: User ${event.userId} completed payment of $${event.amount}`,
      );
    } catch (error) {
  ...
  async handlePaymentFailed(event: PaymentFailedEvent): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        `Payment failed analytics: User ${event.userId} failed payment of amount unknown, reason: ${event.reason}`,
      );
  ...
  async handlePaymentRetryScheduled(event: PaymentRetryScheduledEvent): Promise<void> {
    this.logger.log(
      `Handling payment retry scheduled event: ${event.paymentId}, retry at: ${event.retryAt}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `Scheduled retry notification for payment ${event.paymentId} at ${event.retryAt}`,
      );
    } catch (error) {
  ...
  async handlePaymentRetrySucceeded(event: {
    paymentId: string;
    retryCount: number;
  }): Promise<void> {
    this.logger.log(
      `Handling payment retry succeeded event: ${event.paymentId}, attempt: ${event.retryCount}`,
    );

    try {
      // Log successful retry for analytics
      this.logger.log(
        `Payment retry analytics: Payment ${event.paymentId} succeeded on attempt ${event.retryCount}`,
      );
    } catch (error) {
  ...
  async handlePaymentRetryExhausted(event: PaymentRetryExhaustedEvent): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        `Payment retry exhausted: User ${event.userId}, Payment ${event.paymentId} - requires manual intervention`,
      );
      ...
      if (user) {
        ...
        this.logger.log(
          `Account flagged for review due to payment retry exhaustion: ${event.userId}`,
        );
      }
    } catch (error) {
  ...
  async handlePaymentFinalFailure(event: {
    paymentId: string;
    transactionId: string;
    userId: string;
    reason: string;
  ...
  async handlePaymentMethodSetup(event: {
    userId: string;
    customerId: string;
    ...
    try {
      ...
      this.logger.log(
        `Updated ${pendingPayments.length} pending payments with new payment method for user ${event.userId}`,
      );
    } catch (error) {
