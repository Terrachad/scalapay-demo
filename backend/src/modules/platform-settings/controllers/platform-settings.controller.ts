...
export class PlatformSettingsController {
  ...
  @ApiResponse({ status: 200, description: 'Settings retrieved successfully', type: SuccessResponse })
  ...
  async getAllSettings(
    ...
    try {
      ...
      const totalSettings = Object.values(finalSettings).reduce((count, category) => count + Object.keys(category).length, 0);
      this.logger.log(`Successfully retrieved ${totalSettings} platform settings across ${Object.keys(finalSettings).length} categories`);
  ...
  async getSettingByKey(
    ...
    const context = this.createRequestContext(request);
    const value = await this.settingsService.getSettingByKey(key, environment || context.environment);
  ...
  async validateSettings(
    ...
    try {
      ...
      this.logger.log(`Validation completed: ${results.filter(r => r.valid).length} valid, ${errors.length} errors`);
  ...
  private createRequestContext(request: any): RequestContext {
    return {
      ...
      environment: request?.headers?.['x-environment'] || process.env.NODE_ENV || Environment.DEVELOPMENT
    };
  }
  ...
  async resetToDefaults(
    @Req() request?: any
  ): Promise<SuccessResponse<PlatformSettings>> {
    try {
      const context = this.createRequestContext(request);
      const userId = request?.user?.id || 'unknown-user';
