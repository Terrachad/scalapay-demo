...
export class EnterprisePaymentSchedulerService {
    ...
    try {
      ...
      if (config.enableValidation !== false) {
        ...
        if (!scheduleValidation.isValid) {
          throw new Error(
            `Payment schedule validation failed: ${scheduleValidation.errors.join(', ')}`,
          );
        }
      }
      ...
      this.logger.log(
        `Successfully created payment schedule: ${scheduleId} with ${payments.length} payments`,
      );
      return result;
    } catch (error) {
      ...
      this.logger.error(
        `Failed to create payment schedule for transaction ${transaction.id}`,
        error,
      );
  ...
  private async calculateEnterprisePaymentSchedule(
    ...
    const remainder =
      Math.round((totalAmount - baseInstallmentAmount * installmentCount) * 100) / 100;
    ...
    for (let i = 0; i < installmentCount; i++) {
      ...
      const installmentAmount =
        i === installmentCount - 1 ? baseInstallmentAmount + remainder : baseInstallmentAmount;
  ...
  private async createPaymentRecords(
    ...
    for (const installment of schedule) {
      ...
      payment.status =
        installment.installmentNumber === 1 ? PaymentStatus.PROCESSING : PaymentStatus.SCHEDULED;
  ...
  async repairCorruptedPaymentSchedule(transactionId: string): Promise<PaymentScheduleResult> {
    ...
    try {
      ...
      await queryRunner.rollbackTransaction();
      this.logger.error(
        `Failed to repair payment schedule for transaction ${transactionId}`,
        error,
      );
      throw error;
    } finally {
