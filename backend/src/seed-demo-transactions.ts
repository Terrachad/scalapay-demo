  ...
  const demoTransactions = [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Screen Protector', price: 15.0, quantity: 1 },
        { name: 'Charging Cable', price: 45.0, quantity: 1 },
      ],
      riskScore: 15,
      userId: customers[0].id,
    ...
    },
    {
      ...
      items: [{ name: 'Tablet', price: 599.0, quantity: 1 }],
      riskScore: 65,
      userId: customers[1] ? customers[1].id : customers[0].id,
      merchantId: merchants[0].id,
    },
  ...
  for (const transactionData of demoTransactions) {
    try {
      ...
      if (!existingTransaction) {
        ...
        console.log(
          `Created demo transaction: ${transactionData.amount} - ${transactionData.status}`,
        );
      }
    } catch (error) {
