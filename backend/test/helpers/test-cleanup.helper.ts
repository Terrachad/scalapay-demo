...
export class TestCleanupHelper {
  ...
  async cleanup(tracker: TestCleanupTracker): Promise<void> {
    try {
      ...
      console.log(
        `Cleaned up test data: ${tracker.userIds.length} users, ${tracker.transactionIds.length} transactions, ${tracker.paymentIds.length} payments`,
      );
    } catch (error) {
  ...
  private async batchDelete(
    ...
    for (let i = 0; i < ids.length; i += batchSize) {
      ...
      await this.dataSource.query(`DELETE FROM ${tableName} WHERE id IN (${placeholders})`, batch);
    }
  }
