...
export default function AdminAnalyticsPage() {
  ...
  const conversionRate =
    analytics?.totalTransactions && analytics.totalTransactions > 0
      ? (((analytics.completedTransactions || 0) / analytics.totalTransactions) * 100).toFixed(1)
      : '0';

  const avgTransactionValue =
    analytics?.completedTransactions && analytics.completedTransactions > 0
      ? (analytics.completedRevenue || 0) / analytics.completedTransactions
      : 0;
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {analytics?.completedTransactions || 0} transactions
                  </p>
                </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {analytics?.paymentPlanStats &&
                      Object.entries(analytics.paymentPlanStats).map(([plan, count]) => {
                        const percentage =
                  ...
                  {analytics?.topMerchants?.map((merchant, index) => (
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-gray-500">
                          Commission: {formatCurrency(merchant.revenue * 0.025)}
                        </p>
                      </div>
                    </div>
                  ))}
                  ...
                  <p className="text-3xl font-bold text-purple-600">
                    {formatCurrency(
                      (analytics?.completedRevenue || 0) /
                        Math.max(analytics?.merchantCount || 1, 1),
                    )}
                  </p>
                      ...
                      <div className="text-right">
                        ...
                        <div className="flex items-center gap-2">
                          <Badge variant="secondary">
                            Avg:{' '}
                            {formatCurrency(merchant.revenue / Math.max(merchant.transactions, 1))}
                          </Badge>
                        </div>
                      </div>
                    </div>
